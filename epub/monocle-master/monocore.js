...
Gala.onContact = function (elem, fns, useCapture, initCallback) {
    ...
    var wrapContact = function (fn) {
        ...
        isLeftClick(evt));
            if (doCallFunc) {
                /*INHO - written 2014.05.12*/
                if (evt.currentTarget.className && evt.currentTarget.className == 'monelem_controls_magnifier_button') {
                    Gala.stop(evt);
                }
                /*INHO*/
                fn(evt);
...
Monocle.Dimensions.Columns = function (pageDiv) {
  ...
  function setColumnWidth() {
    ...
    /* INHO - ADDED : S */
    cer += Monocle.Browser.css.toCSSDeclaration('font-size', '13pt');
    cer += Monocle.Browser.css.toCSSDeclaration('line-height', '180%');
    cer += Monocle.Browser.css.toCSSDeclaration('font-family', '"¸¼Àº°íµñ", "±¼¸²Ã¼", "¹ÙÅÁÃ¼"');
    /* INHO - ADDED : E */
